...
export interface SleepRecord {
  value: string      // Category value (e.g., "HKCategoryValueSleepAnalysisAsleepCore")
 ...
 */
function getDateKey(endDateIso: string): string {
 ...
 */
function computeDurationHours(startDate: string, endDate: string): number {
 ...
 */
export function aggregateSleep(records: SleepRecord[]): MetricRow[] {
  if (records.length === 0) {
    log.info('No sleep records to aggregate')
    return []
  }
  ...
  for (const record of records) {
    if (ASLEEP_CATEGORIES.has(record.value)) {
      ...
      if (hours < 0 || hours > 24) {
        log.warn('Invalid sleep duration', { 
          hours, 
  ...
  log.info('Sleep records processed', {
  ...
  if (hoursPerDay.size === 0) {
    log.info('No valid sleep days computed')
    return []
  }
  ...
  if (rows.length > 0) {
    ...
    log.info('Sleep aggregation complete', {
