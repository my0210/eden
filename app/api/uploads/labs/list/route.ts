...
export async function GET() {
  try {
    ...
    const { data: uploads, error } = await supabase
      ...
      .select('id, file_name, file_type, status, lab_date, lab_provider, extracted_values, analysis_metadata, created_at, processed_at')
      .eq('user_id', user.id)
    ...
    const enriched = (uploads || []).map((u: any) => ({
      ...u,
      confirmed: !!u?.analysis_metadata?.confirmed,
    }))
