---
name: Eden Coaching System
overview: Build a comprehensive AI coaching system where users conversationally commit to goals with Eden, follow structured protocols (milestones + weekly actions + habits), and track progress through quick actions and periodic check-ins.
todos:
  - id: data-model
    content: Create coaching tables + DROP eden_plans/eden_plan_actions
    status: completed
  - id: replace-plan-usages
    content: Update buildEdenContext, chat/page, reset-user to use protocols
    status: completed
  - id: delete-old-plan-code
    content: Delete lib/plans/createWeeklyPlanForUser.ts
    status: completed
  - id: goal-extraction
    content: Build goal extraction (includes constraint capture at commitment)
    status: completed
  - id: constraint-detection
    content: Build ongoing constraint detection from chat messages
    status: completed
  - id: refactor-context-builder
    content: Refactor buildEdenContext (remove legacy, add goal/protocol loading)
    status: completed
  - id: protocol-generation
    content: Implement protocol generation (milestones, weekly actions, daily habits)
    status: completed
  - id: versioning-system
    content: Build protocol versioning (version chain, diffing, parent linking)
    status: completed
  - id: decision-logging
    content: Implement decision logging (trigger, reason, changes, expected outcome, re-eval)
    status: completed
  - id: coaching-ui
    content: Build coaching dashboard (milestone timeline, weekly adherence, action/habit UI)
    status: completed
  - id: change-history-ui
    content: Build change history UI (why plan changed, diff view, outcome tracking)
    status: completed
  - id: api-endpoints
    content: Create API endpoints (complete action, log habit, decisions history)
    status: completed
  - id: coaching-loop
    content: Implement coaching loop (check-ins, adaptation with versioning, re-evaluation)
    status: completed
  - id: navigation-integration
    content: Add coaching tab to navigation and integrate with existing chat
    status: completed
---
...
| Table | Purpose |
|-------|---------|
| `eden_goals` | User's committed goals with type, target, timeline |
| `eden_protocols` | Generated protocol tied to a goal (phases, duration) - versioned |
| `eden_protocol_decisions` | Decision log: why protocol changed, expected outcome, re-eval date |
| `eden_milestones` | Checkpoints within a protocol (e.g., "Week 4: Complete first 5K") |
| `eden_protocol_actions` | Weekly/daily actions with completion tracking |
| `eden_habits` | Daily habits to track with streaks |
| `eden_habit_logs` | Individual habit completion entries |
| `eden_checkins` | Periodic check-in responses (weekly reviews, milestone reviews) |

### Tables to DELETE (replaced by above)

| Old Table | Reason |
|-----------|--------|
| `eden_plans` | Replaced by `eden_protocols` (versioned, goal-linked) |
| `eden_plan_actions` | Replaced by `eden_protocol_actions` (with completion tracking) |
...
The following safety rails fields were **defined but never written** - remove them:

| File | Change |
|------|--------|
| [`lib/context/buildEdenContext.ts`](lib/context/buildEdenContext.ts) | Remove `EdenSafetyRails` type fields (diagnoses, meds, injuries_limitations, red_lines, doctor_restrictions). Keep only `privacy_ack`. Load goal constraints instead. |
| [`lib/context/buildEdenContext.ts`](lib/context/buildEdenContext.ts) | Update `summarizeContextForCoach()` to use goal constraints, remove dead safety_rails references |
| [`lib/onboarding/getUserState.ts`](lib/onboarding/getUserState.ts) | Simplify `SafetyJson` type to only `privacy_ack` |
...
|---------|---------|--------|
| 1. Load eden_user_state | Keep | Add goals_json loading |
| 2. Load scorecard | Keep | No change |
| 3. Load uploads | Keep | No change |
| 4. Check first chat | Keep | No change |
| 5. Legacy profile | **DELETE** | Remove eden_user_profile query |
| 6. Persona | **DELETE** | Remove eden_user_personas query |
| 7. Active plan | **REPLACE** | Load active goal + protocol instead |
| NEW 8. Active goal | Add | Load from eden_goals where status='active' |
| NEW 9. Active protocol | Add | Load from eden_protocols (latest version for goal) |
| NEW 10. Weekly adherence | Add | Calculate from eden_protocol_actions + eden_habit_logs |

### summarizeContextForCoach (updated)

| Section | Current | Change |
|---------|---------|--------|
| Essentials | Keep | No change |
| Focus | Keep | No change |
| Safety Rails | **DELETE** | Dead code, never populated |
| Prime Scorecard | Keep | No change |
| Uploads | Keep | No change |
| Weekly plan | **REPLACE** | Show active goal + protocol status |
| NEW: Active Goal | Add | Goal description, target, constraints |
...
| File | Current | Change |
|------|---------|--------|
| [`lib/plans/createWeeklyPlanForUser.ts`](lib/plans/createWeeklyPlanForUser.ts) | Creates `eden_plans` | **DELETE** - replaced by `lib/coaching/generateProtocol.ts` |
| [`lib/context/buildEdenContext.ts`](lib/context/buildEdenContext.ts) | Loads active plan into context | Update to load active goal + protocol |
| [`app/chat/page.tsx`](app/chat/page.tsx) | Shows "This week's focus" from plan | Update to show active goal + current protocol actions |
| [`app/api/dev/reset-user/route.ts`](app/api/dev/reset-user/route.ts) | Deletes `eden_plans` + actions | Update to delete goals, protocols, habits, etc. |
...
2. **Different data model** - protocols are versioned, goal-linked, with milestones; plans were flat weekly snapshots
3. **Simpler code** - one system, one code path, one source of truth